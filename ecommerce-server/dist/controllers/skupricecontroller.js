...
exports.checkout = async items => {
  logger.info("[+] Total number of items checkedout ".concat(items.length));

  try {
    const result = ajv.validate(checkoutSchema, items);
    logger.info("[+] Schema validation result is ".concat(result));
...
//# sourceMappingURL=skupricecontroller.js.map