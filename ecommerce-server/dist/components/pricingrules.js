...
class PricingRules {
  ...
  constructor(skus) {
    ...
    skus.forEach(data => {
      logger.info("[+] ECommerce service has ".concat(data.sku, ", ").concat(data.name, ", ").concat(data.price, ", ").concat(data.img, "."));
    });
    this.products = skus;
    logger.info("[+] Total number of products in pricingRules ".concat(this.products.length));
  }
  /**
  ...
  calculateTotal(items) {
    logger.info("[+] Total number of checkedout items in pricingRules ".concat(items.length)); // 1. We're going to have a 3 for 2 deal on Apple TVs. For example, if you buy 3 Apple TVs, you will pay the price of 2 only
